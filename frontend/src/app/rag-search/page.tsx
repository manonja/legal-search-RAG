...
const QueryInputForm = ({
    ...
    >
      <div className={`flex-1 border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow ${isSticky ? '' : 'max-w-3xl mx-auto'}`}>
...
export default function RagSearchPage() {
  ...
  if (conversationTurns.length === 0) {
    ...
    return (
      <div className="container mx-auto px-4 max-w-4xl pt-16" data-testid="landing-page">
        <section className="text-center mb-12">
          <h1 className="text-4xl text-gray-800 font-bold mb-4">
            Legal Assistant
          </h1>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Ask questions about legal documents and get AI-powered answers with source references.
          </p>
        </section>
        <QueryInputForm
            isSticky={false}
    ...
    return (
      <div className="flex flex-col h-[calc(100vh-theme(spacing.16))] bg-gray-50" data-testid="chat-interface">
        <header className="text-center py-4 border-b border-gray-200 bg-white sticky top-0 z-20">
          <h1 className="text-xl text-gray-800 font-semibold">Legal Assistant</h1>
        ...
        {/* Conversation Area */}
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6" data-testid="conversation-area">
          {conversationTurns.map((turn) => (
            <div key={turn.id} className="space-y-4" data-testid={`turn-${turn.id}`}>
              {/* User Query - Centered */}
              <div className="flex justify-center">
                <div className="bg-blue-400 text-white p-3 rounded-lg max-w-xl shadow mx-auto" data-testid="user-query">
              ...
              {(turn.isLoading || turn.error || turn.assistantResponse) && (
                <div className="flex justify-center">
                  <div className="bg-white border border-gray-200 p-4 rounded-lg max-w-xl shadow-sm w-full mx-auto" data-testid="assistant-response-area">
                    ...
                    {turn.assistantResponse && !turn.isLoading && !turn.error && (
                        <div data-testid="assistant-content">
                          ...
                          {turn.assistantResponse.confidence !== null && (
                              <div className="mt-3 pt-3 border-t border-gray-100 flex items-center gap-2 text-xs" data-testid="confidence-section">
                                ...
                                <div className="flex-1 bg-gray-200 rounded-full h-1.5">
                                  <div className={`h-1.5 rounded-full ${ turn.assistantResponse.confidence > 0.8 ? "bg-green-500" : turn.assistantResponse.confidence > 0.5 ? "bg-yellow-500" : "bg-red-500" }`} style={{ width: `${turn.assistantResponse.confidence * 100}%` }}></div>
                                </div>
                                <span className={`font-semibold ${ turn.assistantResponse.confidence > 0.8 ? "text-green-600" : turn.assistantResponse.confidence > 0.5 ? "text-yellow-600" : "text-red-600" }`} data-testid="confidence-label">
                                  {turn.assistantResponse.confidence > 0.8 ? "High" : turn.assistantResponse.confidence > 0.5 ? "Medium" : "Low"} ({(turn.assistantResponse.confidence * 100).toFixed(0)}%)
                                </span>
                              </div>
                          )}
                          {/* Feedback */}
                          <div className="mt-3 pt-3 border-t border-gray-100 flex items-center gap-2" data-testid="feedback-section">
                            ...
                            <button onClick={() => handleFeedback(turn.id, 'up')} className={`p-1 rounded hover:bg-green-100 disabled:opacity-50 disabled:cursor-not-allowed ${turn.feedback === 'up' ? 'bg-green-100' : ''}`} title="Good answer" disabled={!!turn.feedback} data-testid="feedback-up-button">
                              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5" /></svg>
                            </button>
                            <button onClick={() => handleFeedback(turn.id, 'down')} className={`p-1 rounded hover:bg-red-100 disabled:opacity-50 disabled:cursor-not-allowed ${turn.feedback === 'down' ? 'bg-red-100' : ''}`} title="Bad answer" disabled={!!turn.feedback} data-testid="feedback-down-button">
                               <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M10 14H5.236a2 2 0 01-1.789-2.894l3.5-7A2 2 0 018.738 3h4.017c.163 0 .326.02.485.06L17 4m-7 10v5a2 2 0 002 2h.095c.5 0 .905-.405.905-.905 0-.714.211-1.412.608-2.006L17 9V4m-7 10h2m-2 0H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5" /></svg>
                            </button>
