...
class QueryRequest(BaseModel):
    ...
    min_similarity: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Minimum similarity threshold (0 to 1)"
    )
...
async def search_documents(request: QueryRequest):
    ...
    filtered_results = [
        doc for doc in MOCK_DOCUMENTS if doc["similarity"] >= request.min_similarity
    ][: request.n_results]
    ...
    return QueryResponse(results=results, total_found=len(results))
